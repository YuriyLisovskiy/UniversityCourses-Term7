...
train(
    ...
    [
        ...
        station(station2, "2019-03-04T13:40Z", "2019-03-04T13:50Z"),
        station(station3, "2019-03-04T14:30Z", "2019-03-04T14:40Z"),
...
train(
    ...
    [
        ...
        station(station5, "2019-03-04T08:40Z", "2019-03-04T08:50Z"),
        station(station6, "2019-03-04T11:30Z", "2019-03-04T11:40Z"),
...
train(
    ...
    [
        ...
        station(station2, "2019-03-04T14:30Z", "2019-03-04T14:40Z"),
        station(station3, "2019-03-04T15:30Z", "2019-03-04T15:40Z"),
...
train(
    ...
    [
        ...
        station(station4, "2019-03-04T12:10Z", "2019-03-04T12:20Z"),
...
train(
    ...
    [
        ...
        station(station2, "2019-03-04T02:00Z", "2019-03-04T02:10Z"),
        station(station4, "2019-03-04T14:30Z", "2019-03-04T14:40Z"),
...
train(
    ...
    [
        ...
        station(station1, "2019-03-04T09:00Z", "2019-03-04T09:10Z"),
        station(station2, "2019-03-04T10:20Z", "2019-03-04T10:30Z"),
...
train(
    ...
    [
        ...
        station(station1, "2019-03-04T08:00Z", "2019-03-04T08:10Z"),
        station(station2, "2019-03-04T09:30Z", "2019-03-04T09:40Z"),
        station(station4, "2019-03-04T10:50Z", "2019-03-04T11:00Z"),
...
train(
    ...
    [
        ...
        station(station5, "2019-03-04T11:30Z", "2019-03-04T11:40Z"),
        station(station4, "2019-03-04T13:00Z", "2019-03-04T13:10Z"),
        station(station6, "2019-03-04T15:20Z", "2019-03-04T15:30Z"),
...
train(
    ...
    [
        ...
        station(station6, "2019-03-04T12:00Z", "2019-03-04T12:10Z"),
...
train(
    ...
    [
        ...
        station(station7, "2019-03-04T19:00Z", "2019-03-04T19:10Z"),
        station(station6, "2019-03-04T21:10Z", "2019-03-04T21:20Z"),
...
% 
% Example:
% get_available_places_from_to_by_train(station(lviv, _, _), station(rivne, _, _), 1, X).
% 
% get_available_places_from_to_by_train(
% 	From -> station(...),
% 	To -> station(...),
% 	TrainNumber,
% 	AvailableCabs -> out
% ).
get_available_places_from_to_by_train(From, To, TrainNumber, AvailableCabs) :-
    train(TrainNumber, _, _, Cabs),
    get_cabs_without_occupied_places_from_to(TrainNumber, From, To, Cabs, AvailableCabs).

% End recursion
get_cabs_without_occupied_places_from_to(_, _, _, [], CabsOut) :- CabsOut = [].

get_cabs_without_occupied_places_from_to(TrainNumber, From, To, [CurrentCab | OtherCabs], CabsOut) :-
    get_cabs_without_occupied_places_from_to(TrainNumber, From, To, OtherCabs, OtherCabsOut),
    CurrentCab = cab(CabNumber, Type, Price, AllPlaces),
    get_available_places_from_to(TrainNumber, CabNumber, From, To, AllPlaces, AvailablePlaces),
    CabsOut = [cab(CabNumber, Type, Price, AvailablePlaces) | OtherCabsOut].

% End recursion
get_available_places_from_to(_, _, _, _, [], AvailablePlaces) :- AvailablePlaces = [].

get_available_places_from_to(TrainNumber, CabNumber, From, To, [CurrentPlace | OtherPlaces], AvailablePlaces) :-
	% Check place availability for each place in cab
    get_available_places_from_to(TrainNumber, CabNumber, From, To, OtherPlaces, OtherAvailablePlaces),
...
get_available_places(
    ...
    get_available_places_from_to_by_train(FirstStation, LastStation, CurrentTrainNumber, AvailablePlaces),
...
get_available_places(
    ...
    get_available_places_from_to_by_train(FirstStation, LastStation, CurrentTrainNumber, AvailableCabs),
    AvailablePlaces = [AvailableCabs | OtherAvailablePlaces],
    !.

% Example:
% get_transit_and_available_places(lviv, rivne, 1, "2019-03-04T11:30Z", X).
get_transit_and_available_places(From, To, NumTransfers, MinTime, Result) :-
...
% Example:
% get_tickets_from_to_by_type(lviv, rivne, 1, "2019-03-04T11:30Z", open, X).
get_tickets_from_to_by_type(From, To, NumTransfers, MinTime, CabType, Result) :-
    get_transit_and_available_places(From, To, NumTransfers, MinTime, [Transit | [AvailablePlaces]]),
